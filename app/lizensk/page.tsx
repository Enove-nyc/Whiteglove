    ...
    <main className="min-h-screen bg-[var(--cream)]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <div className="grid gap-10 lg:grid-cols-[.85fr_1.15fr] lg:items-start">
          <div><p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--gold-ink)]">About the tzaddik</p><h2 dir="rtl" lang="yi" className="mt-4 font-[family-name:var(--font-display)] text-4xl leading-tight text-[var(--navy)] sm:text-5xl">רבי אלימלך מליזענסק</h2><p className="mt-3 font-[family-name:var(--font-display)] text-2xl leading-tight text-stone-500 sm:text-3xl">Rabbi Elimelech Weisblum of Lizhensk</p><a href={placeDirectionsUrl("Górna 16, 37-300 Leżajsk, Poland", "50.251139, 22.422611")} target="_blank" rel="noreferrer" className="mt-6 inline-block bg-[var(--navy)] px-5 py-3 text-xs font-bold uppercase tracking-[0.14em] text-white transition hover:bg-[var(--gold)]">Navigate to the kever →</a><Link href="/cemeteries/lizhensk" className="mt-5 block text-xs font-bold uppercase tracking-[0.14em] text-[var(--navy)] underline decoration-[var(--gold)] decoration-2 underline-offset-4">View this בית החיים →</Link><SavePlaceButtons place={{ id: "lizensk", name: "Lizhensk", yiddishName: "ליזענסק", address: "Górna 16, 37-300 Leżajsk, Poland", coordinates: "50.251139, 22.422611", href: "/lizensk" }} /><SuggestEditButton targetType="location" targetId="lizensk" title="Lizhensk" currentInfo="ליזענסק
Rabbi Elimelech Weisblum of Lizhensk
Górna 16, 37-300 Leżajsk, Poland" /><div className="mt-8 border-t border-[var(--gold-light)] pt-5"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--gold-ink)]">Finding the kever</p><ol className="mt-4 space-y-3"><li className="flex gap-3 text-sm leading-6 text-stone-600"><span className="font-semibold text-[var(--gold-ink)]">1.</span><span>Navigate to Górna 16, the Lizhensk Jewish Cemetery.</span></li><li className="flex gap-3 text-sm leading-6 text-stone-600"><span className="font-semibold text-[var(--gold-ink)]">2.</span><span>The kever of Reb Elimelech is within the ohel at the cemetery.</span></li><li className="flex gap-3 text-sm leading-6 text-stone-600"><span className="font-semibold text-[var(--gold-ink)]">3.</span><span>For a yahrzeit visit, expect special access and crowd arrangements; confirm them before you travel.</span></li></ol></div></div>
          <div className="border-l border-[var(--gold)] pl-6"><p className="text-lg leading-8 text-stone-600">Reb Elimelech of Lizhensk was a central early Chassidic leader whose seforim and talmidim shaped generations. His sefer, <em>Noam Elimelech</em>, is a foundational work on the Torah. His yahrzeit is 21 Adar; he was niftar in 5547 (1787).</p><div className="mt-8 grid gap-5 sm:grid-cols-3"><div><p className="text-xs font-bold uppercase tracking-[0.17em] text-[var(--gold-ink)]">Sefer</p><p dir="rtl" lang="yi" className="mt-2 font-[family-name:var(--font-display)] text-2xl text-[var(--navy)]">נועם אלימלך</p></div><div><p dir="rtl" lang="yi" className="text-xs font-bold tracking-[0.12em] text-[var(--gold-ink)]">יארצייט</p><p dir="rtl" lang="yi" className="mt-2 font-[family-name:var(--font-display)] text-2xl text-[var(--navy)]">כ״א אדר</p></div><div><p dir="rtl" lang="yi" className="text-xs font-bold tracking-[0.12em] text-[var(--gold-ink)]">שנת פטירה</p><p dir="rtl" lang="yi" className="mt-2 font-[family-name:var(--font-display)] text-2xl text-[var(--navy)]">תקמ״ז / 1787</p></div></div></div>
        </div>
      </section>
      ...
      <section id="tefillos" className="border-y border-[var(--gold-light)] bg-[var(--cream-deep)] px-5 py-20 sm:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
          <div><p className="text-xs font-bold uppercase tracking-[0.24em] text-[var(--gold-ink)]">Prepare with peace of mind</p><h2 className="mt-4 font-[family-name:var(--font-display)] text-4xl leading-tight text-[var(--navy)] sm:text-5xl">Make room for what brought you here.</h2><p className="mt-6 leading-8 text-stone-600">This guide will collect practical preparation for the visit, while leaving room for every person to come with their own tefillos and intention.</p></div>
          <div className="border border-[var(--gold-light)] bg-[#fcfaf6] p-7 sm:p-9"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--gold-ink)]">Before you go</p><ul className="mt-5 space-y-4">{preparationChecklist.map((item) => <li key={item} className="flex gap-4 border-t border-[var(--gold-light)] pt-4 text-stone-700 first:border-t-0 first:pt-0"><span className="mt-1 text-[var(--gold-ink)]">✦</span><span className="leading-7">{item}</span></li>)}</ul></div>
        </div>
      </section>
      <section id="contacts" className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2">
          {verifiedDetails(contactEmail).map((item) => <article key={item.title} className="border border-[var(--gold-light)] bg-[#fcfaf6] p-7"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--gold-ink)]">{item.label}</p><h2 className="mt-3 font-[family-name:var(--font-display)] text-3xl text-[var(--navy)]">{item.title}</h2><p className="mt-4 leading-7 text-stone-600">{item.detail}</p><a className="mt-6 inline-block text-xs font-bold uppercase tracking-[0.15em] text-[var(--navy)] underline decoration-[var(--gold)] decoration-2 underline-offset-4 transition hover:text-[var(--gold-ink)]" href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel={item.href.startsWith("http") ? "noreferrer" : undefined}>{item.action} →</a></article>)}
        </div>
        <div className="mt-8 border border-[var(--gold-light)] bg-[var(--navy)] p-8 text-white sm:flex sm:items-center sm:justify-between sm:gap-8"><div><p className="text-xs font-bold uppercase tracking-[0.22em] text-[var(--gold-light)]">Help us keep this useful</p><p className="mt-2 text-lg text-slate-100">Have a trusted driver, food contact, or current minyan detail for Lizhensk?</p></div><a href={`mailto:${contactEmail}?subject=Lizhensk%20information`} className="mt-5 inline-block shrink-0 border border-[var(--gold-light)] px-6 py-4 text-center text-sm font-bold uppercase tracking-[0.14em] transition hover:bg-[var(--gold)] hover:text-[var(--navy-deep)] sm:mt-0">Share a trusted contact</a></div>
      </section>
      <Footer />
    </main>
  );
