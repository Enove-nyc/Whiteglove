...
export async function GET() {
  ...
  if (!email) return NextResponse.json({ signedIn: false, turns: [] });
  const conversation = readConversation(await getAssistantConversation(email));
  return NextResponse.json({ signedIn: true, turns: conversation.turns });
}
...
export async function POST(request: NextRequest) {
  ...
  if (!added.length) return NextResponse.json({ signedIn: true, saved: false });
