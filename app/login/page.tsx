  ...
  return (
    <main className="min-h-screen bg-[var(--cream)]">
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-5 py-20 sm:px-8 lg:grid-cols-[.9fr_1.1fr] lg:py-28">
        ...
        <div className="border border-[var(--gold-light)] bg-[#fcfaf6] p-8 shadow-[0_12px_30px_rgba(23,45,82,.08)] sm:p-10"><p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--gold)]">Sign in or create an account</p><h2 className="mt-4 font-[family-name:var(--font-display)] text-4xl text-[var(--navy)]">Your personal travel book.</h2><form className="mt-8 space-y-5"><label className="block text-sm font-semibold text-[var(--navy)]">Email address<input type="email" placeholder="you@example.com" className="mt-2 w-full border border-[var(--gold-light)] bg-white px-4 py-3 outline-none focus:border-[var(--gold)]" /></label><button type="button" className="w-full bg-[var(--navy)] px-5 py-4 text-sm font-bold uppercase tracking-[0.14em] text-white transition hover:bg-[var(--gold)]">Continue with email</button></form><p className="mt-6 text-sm leading-6 text-stone-600">Secure sign-in and saved itineraries are the next connection we are preparing. Your information will remain private to your account.</p></div>
      </section>
      <Footer />
    </main>
  );
