...
describe("the words, exactly as the owner set them", () => {
  it("keeps the homepage label and its supporting text", () => {
    ...
    assert.equal(
      ASSISTANT_HOME_SUPPORT,
      "Get ideas and help exploring the site. Answers are generated by AI and may not have been reviewed or verified by White Glove.",
    );
  });
  ...
  it("keeps the notice beside the input and the label on an answer", () => {
    assert.equal(
      ASSISTANT_INPUT_NOTICE,
      "AI-generated answers may be inaccurate or out of date. Confirm important kosher and travel details directly.",
    );
    assert.equal(ANSWER_LABEL, "AI-generated · Not reviewed by White Glove");
    assert.equal(SOURCED_LABEL, "Based on published White Glove information");
  });
});
