...
describe("nothing is invisible", () => {
  it("describes every environment variable the code actually reads", () => {
    ...
    // DEPLOYMENT_SETTINGS counts as described: it is the same shape and shown
    // on the same screen, and is separate only because a brand flag is not a
    // connection to anything — no key, no account, nobody to sign up with.
