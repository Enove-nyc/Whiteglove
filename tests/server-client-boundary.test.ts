 ...
 *
 * WHAT THIS COST, IN THE OTHER REPOSITORY. `advisorPlacesFor` was a pure
 * function — a plan in, a filtered list of links out, no hooks and no browser
 * — and it sat in components/AccountMenu.tsx, which is "use client", because
 * that is where the menu using it happened to be. Next turns every export of a
 * client module into a client reference, so importing it on the server gives a
 * marker object rather than the function. The itineraries site's /advisor is a
 * server component and called it, so every request to the advisor's own
 * dashboard threw:
 *
