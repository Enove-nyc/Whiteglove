 ...
 *
 * WHAT THE RULE ACTUALLY IS, since "never read the built-in" would be too
 * strong here: the built-in title is kosher copy, so it is the RIGHT title on
 * the kosher brand and the wrong company on the other. A page may fall back to
 * it once it has established which brand is asking. What no page may do is read
 * it as though the owner had chosen it, which is exactly what `page?.seoTitle ??`
 * does — the fallback written beside it is then unreachable.
 *
...
describe("a page's metadata never inherits the other brand's built-in title", () => {
  ...
  for (const { file, route } of shared) {
    it(`${route} (${file}) settles the brand before it reads a built-in title`, () => {
