...
/**
 * The concierge thread: text, a picture, or a place — and a way to report one.
 *
 ...
 * the fences around the one message a client with no account can push real
 * bytes with — a picture — which are checked in the route source, the same way
 * business-trips.test.ts checks its gates, because the property is that the
 * checks are PRESENT and come before the work.
 */
...
describe("reading a stored thread", () => {
  ...
  it("reads a picture and a place", () => {
    const rows = [
    ...
    assert.equal(out[1].kind, "location");
    assert.equal(out[1].lat, 41.9);
  });

  it("drops a broken picture or place rather than showing an empty bubble", () => {
    const rows = [
...
describe("sending a picture is fenced", () => {
  const ROUTE = readFileSync("app/api/companion/chat/route.ts", "utf8");
  const imageBranch = ROUTE.slice(ROUTE.indexOf("body.dataUrl"), ROUTE.indexOf("A place"));
  ...
  it("is rate limited before it stores anything", () => {
    assert.match(imageBranch, /rateLimit\(`companion-image:/);
    assert.ok(imageBranch.indexOf("rateLimit") < imageBranch.indexOf("putMedia"), "the limit must be counted before the write");
  });

  it("checks the type and the size before it stores anything", () => {
    assert.match(imageBranch, /CHAT_IMAGE_TYPES\.has/);
    assert.match(imageBranch, /effectiveMediaLimit\(\)/);
    assert.ok(imageBranch.indexOf("CHAT_IMAGE_TYPES.has") < imageBranch.indexOf("putMedia"));
    assert.ok(imageBranch.indexOf("effectiveMediaLimit") < imageBranch.indexOf("putMedia"));
  });

  it("only accepts photograph types, not every media type the store takes", () => {
    // The media store also accepts PDFs and GIFs; a chat picture is a photo.
