...
describe("the pass is granted where the money actually lands", () => {
  it("the webhook writes a pass, not only a plan", () => {
    ...
    // Bound to the trip only when that trip is still one of this account's:
    // it can be deleted between reaching Stripe's page and this arriving, and a
    // pass on a trip that is gone is a purchase nobody can use. See
    // tests/trip-pass-ownership.test.ts.
