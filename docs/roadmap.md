...
| 7. Rich media library | **Part** | pictures can be uploaded, captioned, credited and published on a destination, and they appear on the page with the credit under them. A picture with no credit is saved as a draft, enforced on the server as well as the screen. **Still open:** pictures on cemeteries and listings, reordering, and a shared library rather than per-destination uploads. |
