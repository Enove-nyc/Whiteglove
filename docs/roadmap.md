...
| The tab icon (owner request, not in either scan) | **Done** | four goes at this, and the first three were each accurately complained about. `/icon-192.png` was named in the metadata — the one file with a solid navy square baked in, so the tab showed **a blue box**. Removing it fell back to the full logo, which is fine gold line-work: at 32 pixels it still reads and at 16 it dissolves into a smudge, so the tab showed **a gold background**. Replacing it with the compass rose was legible and still wrong — **the rose is the map's mark**, the pin on every marker and the key in its legend, and the site does not name itself with it. It is now the logo, the gloved hand holding the compass, **redrawn at a weight that survives sixteen pixels** rather than shrunk to them: the compass large enough for its rose to register, the hand reduced to the gesture that identifies it. **No background tile** — and it stays readable on a pale tab and a dark one because the artwork itself carries both, the glove and the compass face filled cream and the outline navy, which is the logo's own colouring rather than a plate put behind it. `app/icon.svg` is the only drawing: `scripts/build-favicon.mjs` rasterises `favicon.ico` from that file, and a test rebuilds it and compares, so the `.ico` a browser asks for before it has parsed any HTML cannot fall behind the SVG a modern one uses. |
