...
| The tab icon (owner request, not in either scan) | **Done** | five goes at this, and the first four were each accurately complained about. `/icon-192.png` was named in the metadata — the one file with a solid navy square baked in, so the tab showed **a blue box**. Removing it fell back to the full logo, which is fine gold line-work: at 32 pixels it still reads and at 16 it dissolves into a smudge, so the tab showed **a gold background**. Replacing it with the compass rose was legible and still wrong — **the rose is the map's mark**, the pin on every marker and the key in its legend, and the site does not name itself with it. The fourth was the right shape in the wrong colour: navy, because it was traced from `public/logo-hand-navy.png` — which is a recoloured copy used as a bullet in the page body, not the mark. **In `public/logo.png` only the "White Glove" wordmark is navy; the hand and the compass are gold.** It is now the logo, the gloved hand holding the compass, **in the site's own gold `#aa8b52`**, and **redrawn at a weight that survives sixteen pixels** rather than shrunk to them: the compass large enough for its rose to register, the hand reduced to the gesture that identifies it. Bold, so it reads as a gold *mark* rather than the gold smear the hairlines made. **No background tile** — and it stays readable on a pale tab and a dark one because the artwork itself carries both, the glove and the compass face filled cream and the line-work gold, which is the logo's own colouring rather than a plate put behind it. `app/icon.svg` is the only drawing: `scripts/build-favicon.mjs` rasterises `favicon.ico` from that file, and a test rebuilds it and compares, so the `.ico` a browser asks for before it has parsed any HTML cannot fall behind the SVG a modern one uses. Two more guards, both from mistakes made getting here: the colour must be the logo's gold, and **no comment may contain two hyphens in a row** — an SVG served as `image/svg+xml` is parsed as XML, where that ends the comment and the browser draws nothing at all, silently and only on the tab. |
