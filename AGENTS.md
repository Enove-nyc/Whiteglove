...
- **The White Glove app (`/app`) is for Gold and Business.** App access is the `companionApp` entitlement (`lib/account-limits.ts`), true for Gold and Business — a Gold traveller carries their own trip in it. Handing a trip to a *client* stays Business-only, behind the separate `ownBranding` gate: naming a client, putting an advisor on a trip, and creating a client code are Business only. Opening the app to Gold does not open client codes to them.
- **A client reaches a trip by a per-trip code, not a login.** Each trip has a unique code — its **share token** (a random, unguessable per-trip id from `ensureTripShare`), never the raw internal trip id, so a client cannot guess a nearby one and land on someone else's trip. A Business adviser creates the code and sends it to that one client; it opens that one trip as the app and no other. The `/app` front page, for a visitor not already in their own app, offers two doors: **"Enter a code from your travel adviser"** (a client, no account — opens their one trip) and **"Log in"** (advisers and Gold members, who then see their own trips). Do not turn client access back into an account requirement, and do not use the raw trip id as the code.
