...
- **Two companies, two websites, and they do not merge.** WHITE GLOVE ITINERARIES builds itineraries of every kind, and its customer is the travel adviser. WHITE GLOVE KOSHER TRAVEL is a kosher travel guide, and its customer is the traveller. They are separate businesses on separate sites, served by separate deployments from separate codebases — this repo is White Glove Kosher Travel, and nothing built here reaches whitegloveitineraries.com. Do not propose merging them, do not build one to be served by the other, and do not add links from this site into paths that only exist on the other one. The brand-aware code in this repo (`lib/site-brand-core.ts` and the itineraries lists it feeds) is not a plan to host both.
