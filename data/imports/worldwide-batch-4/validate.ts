...
import { worldwideBatch4Candidates } from "./candidates";
import {
  duplicateKey,
  makeCandidateId,
  normalizedLocation,
  normalizeText,
  WORLDWIDE_BATCH_4_SCHEMA_VERSION,
} from "./schema";
...
export function validatePack() {
  assert.ok(worldwideBatch4Candidates.length >= 4800, `expected >=4800, got ${worldwideBatch4Candidates.length}`);
  const ids = new Set<string>();
  const keys = new Set<string>();
  const byListingLabel: Record<string, number> = {};
  const byImportKind: Record<string, number> = {};

  for (const candidate of worldwideBatch4Candidates) {
    assert.equal(candidate.schemaVersion, WORLDWIDE_BATCH_4_SCHEMA_VERSION);
    assert.equal(candidate.id, makeCandidateId(candidate));
    assert.equal(candidate.sourceId, candidate.id);
    assert.ok(candidate.id.startsWith("wgb4-"), candidate.id);
  ...
  return {
    candidateCount: worldwideBatch4Candidates.length,
    byListingLabel,
    byImportKind,
    byCountry: Object.fromEntries(
      [...worldwideBatch4Candidates.reduce((m, c) => m.set(c.country, (m.get(c.country) ?? 0) + 1), new Map())].sort(
        (a, b) => b[1] - a[1],
      ),
    ),
  };
