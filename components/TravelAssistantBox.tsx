...
export default function TravelAssistantBox({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div
      className={`border border-[var(--gold-light)] bg-[#fcfaf6] ${embedded ? "rounded-2xl p-5 sm:p-6" : "p-5 sm:p-8"}`}
      data-ai-assistant=""
    >
      <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--gold-ink)]">Ask the White Glove assistant</p>
      <h2 className="mt-2 font-[family-name:var(--font-display)] text-[1.75rem] leading-tight text-[var(--navy)] sm:text-3xl">
