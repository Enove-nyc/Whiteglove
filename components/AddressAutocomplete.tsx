...
export default function AddressAutocomplete({
  ...
  /**
   * The suggestions, kept with the query they answer — see LibraryManager for
   * the same shape and the same reason. Clearing them at the top of an effect
   * was the setState the lint rule refuses, and moving that clear behind the
   * 280ms debounce would have left suggestions for a deleted query sitting
   * under whatever was typed next.
   */
  ...
  return (
    <div ref={boxRef} className="relative">
      ...
      {open && results.length > 0 && (
        <ul style={{ ...anchoredStyle(anchor), width: anchor?.width }} className="z-[var(--wg-z-popover)] border border-[var(--gold)] bg-[#FAF8F3] shadow-[0_16px_36px_rgba(23,45,82,.14)]">
          {results.map((s, i) => (
            <li key={`${s.label}-${i}`}>
              <button
                type="button"
                // No clearing needed any more: setQuery makes the box hold the label,
                // which is not the query these rows answer, so `results` above is
