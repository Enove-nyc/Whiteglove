...
export default function MobileBottomBar({ signedIn }: { signedIn: boolean }) {
  ...
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-0 z-[var(--wg-z-header)] flex border-t border-[var(--gold-light)] bg-[#FAF8F3] pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_16px_rgba(23,45,82,.08)] sm:hidden"
    >
      {items.map((item) => {
