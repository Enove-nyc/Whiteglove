...
export default function Navbar() {
  ...
  return (
    <nav className="relative border-b border-[var(--gold-light)] bg-[rgba(247,243,235,0.94)]">
      ...
      {menuOpen && <div className="absolute right-5 top-[4.6rem] z-30 w-60 border border-[var(--gold-light)] bg-[#fcfaf6] p-3 shadow-xl sm:right-8"><Link onClick={() => setMenuOpen(false)} className="block px-4 py-3 text-sm font-semibold text-[var(--navy)] transition hover:bg-[var(--cream-deep)]" href="/">היים <span className="ml-2 text-xs font-normal text-stone-500">Home</span></Link><Link onClick={() => setMenuOpen(false)} className="block px-4 py-3 text-sm font-semibold text-[var(--navy)] transition hover:bg-[var(--cream-deep)]" href="/lizensk">ליזענסק <span className="ml-2 text-xs font-normal text-stone-500">Lizhensk</span></Link><Link onClick={() => setMenuOpen(false)} className="block px-4 py-3 text-sm font-semibold text-[var(--navy)] transition hover:bg-[var(--cream-deep)]" href="/stops">נסיעות <span className="ml-2 text-xs font-normal text-stone-500">Destinations</span></Link><Link onClick={() => setMenuOpen(false)} className="block px-4 py-3 text-sm font-semibold text-[var(--navy)] transition hover:bg-[var(--cream-deep)]" href="/cemeteries">בתי החיים <span className="ml-2 text-xs font-normal text-stone-500">Cemeteries</span></Link><Link onClick={() => setMenuOpen(false)} className="block px-4 py-3 text-sm font-semibold text-[var(--navy)] transition hover:bg-[var(--cream-deep)]" href="/my-route">מיין וועג <span className="ml-2 text-xs font-normal text-stone-500">My Route</span></Link><Link onClick={() => setMenuOpen(false)} className="block px-4 py-3 text-sm font-semibold text-[var(--navy)] transition hover:bg-[var(--cream-deep)]" href="/account">מיין אקאונט <span className="ml-2 text-xs font-normal text-stone-500">My account</span></Link><a onClick={() => setMenuOpen(false)} className="block px-4 py-3 text-sm font-semibold text-[var(--navy)] transition hover:bg-[var(--cream-deep)]" href="#contact">קאָנטאַקט <span className="ml-2 text-xs font-normal text-stone-500">Contact White Glove</span></a></div>}
    </nav>
  );
}
