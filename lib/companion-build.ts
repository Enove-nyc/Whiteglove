...
import { itineraryToCompanionTrip, type CompanionKosherNearby } from "@/lib/companion-trip";
 ...
 * when the account has turned it on.
 *
 ...
 * only when `opts.kosher` is true, works out candle-lighting, when Shabbos
 * ends, and the kosher places near the trip, all offline. Returns null when
 * there is nothing to show yet (no dates, or no days), and the caller decides
 * what to say instead.
 */
export async function buildCompanionFromItinerary(
  ...
  // Only work the layer out when it will be shown — no zmanim, no kosher
  // lookup for an account that keeps the app a plain planner.
  const kosherOn = opts.kosher === true;
  const zmanimByDate: Record<string, ReturnType<typeof zmanimForDay>> = {};
  let kosher: CompanionKosherNearby[] = [];
  if (kosherOn) {
    ...
    kosher = center
      ? curatedKosherPlacesNear({ lat: center.lat, lng: center.lng }, 25)
          .slice(0, 6)
          .map((k) => ({
            name: k.name,
            city: k.city,
