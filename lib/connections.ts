 ...
 */
/**
 * Which brand a deployment serves, when it only ever serves one.
 *
 ...
 * out what a variable is for, so leaving it out would make it invisible in
...
export const DEPLOYMENT_SETTINGS: Connection[] = [
  {
    ...
    without:
      "The navigation renders as White Glove Kosher Travel and corrects itself in the browser a moment later, so a visitor on the itineraries domain sees the wrong name change in front of them — and anything reading the HTML, a search engine or a link preview, never sees it change at all. Set it to `itineraries` on the service serving whitegloveitineraries.com. The kosher deployment can leave it unset.",
    weight: "nicety",
